...
export class SatelliteManager {
  ...
  async addFromTleUrlnc(url, tags) {
    return fetch(url, {
      mode: 'no-cors',
    })
      .then((response) => {
  ...
  add(newSat) {
    const existingSat = this.satellites.find(
      (sat) =>
        sat.props.satnum === newSat.props.satnum &&
        sat.props.name === newSat.props.name
    );
    if (existingSat) {
  ...
  get trackedSatelliteNum() {
    const satellite = this.satellites.find((sat) => sat.isTracked);
    return satellite ? satellite.props.satnum : '';
  }

  set trackedSatelliteNum(num) {
    if (!num) {
      if (this.trackedSatellite) {
        this.viewer.trackedEntity = undefined;
      }
    ...
    if (num === this.trackedSatelliteNum) {
      return;
    }

    const sat = this.getSatelliteNum(num);
    sat.show(this.enabledComponents);
    if (sat) {
      sat.track();
      this.pendingTrackedSatellite = undefined;
  ...
  get enabledSatellitesByNum() {
    return this.enabledSatellites.map((sat) => sat.props.satnum);
  }

  set enabledSatellitesByNum(sats) {
    if (sats) {
      for (var i = 0;i<this.satellites.length;i++) {

        var sat = this.satellites[i];
        if (sats.includes(sat.props.satnum)) {
          try {
            sat.show(this.enabledComponents);
          } catch (error) {
            console.log(error);
  ...
  //     .filter((sat) => sat.props.pm.active)
  ...
  getSatelliteNum(num) {
    return this.satellites.find((sat) => sat.props.satnum === num);
  }
  ...
  disableTag(tag) {
    this.enabledTags = this.enabledTags.filter(
      (enabledTag) => enabledTag !== tag
    );
    this.showSatsWithEnabledTags();
  }
